...
const handler = async (req) => {
  const { personal, medical, counselling, surgery, payments, documents, ops } =
    await req.json();
  ...
  try {
    ...
    if (surgery) {
      ...
      if (surgery.seniorTech)
        addEmployeeUpdate(surgery.seniorTech, "seniorTech");
      if (surgery.implanterRight)
        addEmployeeUpdate(surgery.implanterRight, "implanterRight");
      if (surgery.implanterLeft)
        addEmployeeUpdate(surgery.implanterLeft, "implanterLeft");
      if (surgery.graftingPerson)
        addEmployeeUpdate(surgery.graftingPerson, "graftingPerson");
      
      // Handle helpers array (FIXED: moved after function definition)
