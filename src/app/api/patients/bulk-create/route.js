...
const handler = async (req) => {
  ...
  try {
    ...
    if (surgery.helpers && Array.isArray(surgery.helpers)) {
      surgery.helpers.forEach((helperId) => {
        if (helperId) addEmployeeUpdate(helperId, "helper");
      });
    }
      ...
      if (employeeId) {
        employeeUpdatePromises.push(
          Employee.findByIdAndUpdate(employeeId, {
            ...
            console.error(
              `Error updating employee ${fieldName} with ID ${employeeId} for patient ${patientId}:`,
              error
            );
    ...
    savedPatients.forEach((savedPatient, index) => {
      ...
      if (originalPatient.surgery) {
        if (originalPatient.surgery.doctor)
          addEmployeeUpdate(
            originalPatient.surgery.doctor,
            "doctor",
            savedPatient._id
          );
        if (originalPatient.surgery.seniorTech)
          addEmployeeUpdate(
            originalPatient.surgery.seniorTech,
            "seniorTech",
            savedPatient._id
          );
        if (originalPatient.surgery.implanterRight)
          addEmployeeUpdate(
            originalPatient.surgery.implanterRight,
            "implanterRight",
            savedPatient._id
          );
        if (originalPatient.surgery.implanterLeft)
          addEmployeeUpdate(
            originalPatient.surgery.implanterLeft,
            "implanterLeft",
            savedPatient._id
          );
        if (originalPatient.surgery.graftingPerson)
          addEmployeeUpdate(
            originalPatient.surgery.graftingPerson,
            "graftingPerson",
            savedPatient._id
          );
        if (originalPatient.surgery.helper)
          addEmployeeUpdate(
            originalPatient.surgery.helper,
            "helper",
            savedPatient._id
          );
      }
