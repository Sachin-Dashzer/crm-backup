...
// Hard-deletes a collab settlement, recording it in DeleteLog first.
//
// The settlement's linked transactions go with it, deliberately. Unlike a receivable's receipts
// — which are independent entries that merely POINT at the receivable — these transactions are
// generated BY the settlement in settlements/create and exist only to represent it (the WE_PAID
// expense, or one revenue row per allocated case on THEY_PAID). Leaving them behind would keep
// the money on the books with nothing explaining it, which is the opposite of what deleting the
// settlement means. Everything removed is captured in the DeleteLog entry first.
export async function DELETE(req, { params }) {
  try {
    ...
    // Session-wrapped so a settlement can never be removed while the transactions it generated
    // survive (or vice versa) — the same replica-set transaction approach used by
    // src/lib/collabDerivation.js.
