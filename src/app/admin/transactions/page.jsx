...
function DataTable({ category, rows, onEdit, onDelete, onSort, sortConfig, pagination, onGenerateBill }) {
  ...
  const getExpenseGiverName = (row) => {
    if (row.expenseGiver?.type === "VENDOR") {
      if (typeof row.expenseGiver.vendorId === "object") {
        return row.expenseGiver.vendorId?.name || row.expenseGiver.name || "N/A";
      }
        ...
        {columns.map((col) => (
          <div
            ...
            className={`px-2 py-3 text-left text-xs font-semibold text-slate-700 uppercase tracking-wide truncate ${
              col.sortable ? "cursor-pointer hover:bg-slate-200/50 transition-colors duration-150 group" : ""
            }`}
                    ...
                    <div className="hidden md:grid items-center min-h-16 px-2" style={{ gridTemplateColumns }}>
                      ...
                      {rowCategory === "TRANSPLANT" && (
                        <>
                            ...
                            <div className="text-center">
                              ...
                              {hasDiscount && (
                                <div className="flex items-center justify-center gap-1 mt-1">
                                  ...
                                  <span className="text-xs text-amber-600 font-medium">-{formatCurrency(row.discount)}</span>
                                </div>
                              )}
                            </div>
                          </div>
                          ...
                          <div className="px-2 py-3">
                            {row.paymentId ? (
                              <div className="bg-slate-100 px-2 py-1 rounded text-xs font-mono text-slate-700 truncate">{row.paymentId}</div>
                            ) : (
                      ...
                      {rowCategory === "SERVICE" && (
                        <>
                          ...
                          <div className="px-2 py-3 text-center">
                            <div className="text-sm font-bold text-indigo-700">{row.quantity || 1}</div>
                          </div>
                          <div className="px-2 py-3 text-center">
                            <div className="text-sm font-medium text-slate-900">{formatCurrency(row.perSessionCost || 0)}</div>
                          </div>
                          <div className="px-2 py-3">
                            <div className="text-center">
                              <div className="text-sm font-bold text-emerald-700">{formatCurrency(netAmount)}</div>
                          ...
                          <div className="px-2 py-3">
                            {row.batchId ? (
                              <div className="flex items-center gap-1">
                                ...
                                <span className="text-xs font-mono text-indigo-700">{row.batchId.slice(-8)}</span>
                              </div>
                            ) : (
                              <span className="text-xs text-slate-400">-</span>
                            )}
                      ...
                      {rowCategory === "MEDICINE" && (
                        <>
                          ...
                          <div className="px-2 py-3">
                            <div className="text-sm font-semibold text-slate-900 truncate">{getMedicineName(row)}</div>
                          </div>
                          <div className="px-2 py-3 text-center">
                            <div className="text-sm font-bold text-indigo-700">{row.quantity || 1}</div>
                          </div>
                          <div className="px-2 py-3 text-center">
                            <div className="text-sm font-medium text-slate-900">{formatCurrency(row.perUnitCost || 0)}</div>
                          </div>
                          <div className="px-2 py-3">
                            <div className="text-center">
                              <div className="text-sm font-bold text-emerald-700">{formatCurrency(netAmount)}</div>
                      ...
                      {rowCategory === "EXPENSE" && (
                        <>
                          ...
                          <div className="px-2 py-3">
                            <div className="text-sm text-slate-900 truncate">{getExpenseGiverName(row)}</div>
                          </div>
                          <div className="px-2 py-3 text-right">
                            <div className="text-sm font-bold text-rose-600">{formatCurrency(row.amount)}</div>
                          </div>
                          <div className="px-2 py-3">
                            <span className={`inline-flex px-2 py-1 rounded-lg text-xs font-semibold border ${getMethodColor(row.method)}`}>
                              {row.method?.toUpperCase()}
                    ...
                    <div className="md:hidden p-4">
                      <div className="space-y-4">
                        <div className="flex items-start justify-between">
                          <div>
                            <div className="flex items-center gap-2 mb-1 flex-wrap">
                              ...
                              >
                                {rowCategory === "TRANSPLANT" || rowCategory === "SERVICE"
                                  ? row.procedure
                                  : rowCategory === "MEDICINE"
                                    ? getMedicineName(row)
                                    : row.expense || row.expenseCategory || "Expense"}
                              </span>
                            ...
                            <h4 className="text-base font-bold text-slate-900">
                              {rowCategory !== "EXPENSE" ? getPatientName(row) : getExpenseGiverName(row)}
                            </h4>
                            {rowCategory !== "EXPENSE" && (
                              <p className="text-sm text-slate-600 font-medium">{getPatientPhone(row)}</p>
                            )}
                            {rowCategory === "EXPENSE" && (
                              <p className="text-sm text-slate-600 font-medium">
                                {row.expense || row.expenseCategory || "General Expense"}
                              </p>
                            )}
                          </div>
                          <div className="text-right">
                            <div className={`text-lg font-bold ${rowCategory === "EXPENSE" ? "text-rose-600" : "text-emerald-700"}`}>
                              {formatCurrency(netAmount)}
                            </div>
                        ...
                        <div className="grid grid-cols-2 gap-3">
                          {(rowCategory === "SERVICE" || rowCategory === "MEDICINE") && (
                            <>
                              <div>
                                <p className="text-xs text-slate-500 mb-1">{rowCategory === "SERVICE" ? "Sessions" : "Quantity"}</p>
                                <p className="text-sm font-semibold text-indigo-700">{row.quantity || 1}</p>
                              </div>
                              <div>
                                <p className="text-xs text-slate-500 mb-1">{rowCategory === "SERVICE" ? "Per Session" : "Per Unit"}</p>
                                <p className="text-sm font-semibold text-slate-700">
                                  {formatCurrency(rowCategory === "SERVICE" ? row.perSessionCost : row.perUnitCost)}
                                </p>
                              </div>
                            </>
                          )}
                          {rowCategory === "TRANSPLANT" && (
                            <div>
                              ...
                              <p className="text-sm font-semibold text-blue-700">{row.paymentType || "N/A"}</p>
                            </div>
                          )}
                          {rowCategory === "EXPENSE" && (
                            <div>
                              <p className="text-xs text-slate-500 mb-1">Expense Type</p>
                              <p className="text-sm font-semibold text-rose-700">{row.expense || row.expenseCategory || "N/A"}</p>
                            </div>
                          )}
                          <div>
                            <p className="text-xs text-slate-500 mb-1">Branch</p>
                            <p className="text-sm font-semibold text-purple-700">{row.branch}</p>
                          </div>
                          <div>
                            <p className="text-xs text-slate-500 mb-1">Transaction ID</p>
                            <p className="text-sm font-mono text-slate-700 truncate">{row.paymentId || "-"}</p>
                          </div>
                          {row.batchId && (
                            <div className="col-span-2">
                              <p className="text-xs text-slate-500 mb-1">Batch ID</p>
                              <p className="text-sm font-mono text-indigo-700">{row.batchId.slice(-8)}</p>
                            </div>
                          )}
                        </div>
...
export default function AllTransactionsPage() {
  ...
  const filteredTransactions = useMemo(() => {
    ...
    if (filters.branch) {
      list = list.filter((t) => t.branch?.toLowerCase() === filters.branch.toLowerCase());
    }

    if (filters.paymentMethod) {
      list = list.filter((t) => t.method?.toLowerCase() === filters.paymentMethod.toLowerCase());
    }

    if (filters.procedure) {
      list = list.filter((t) => t.procedure?.toLowerCase() === filters.procedure.toLowerCase());
    }
  ...
  const categoryStats = useMemo(() => {
    ...
    if (filters.branch) {
      filteredList = filteredList.filter((t) => t.branch?.toLowerCase() === filters.branch.toLowerCase());
    }
    if (filters.paymentMethod) {
      filteredList = filteredList.filter((t) => t.method?.toLowerCase() === filters.paymentMethod.toLowerCase());
    }
    if (filters.procedure) {
      filteredList = filteredList.filter((t) => t.procedure?.toLowerCase() === filters.procedure.toLowerCase());
    }
    ...
    if (sortConfig.key) {
      sorted.sort((a, b) => {
        ...
        if (sortConfig.key === "patient") {
          aVal = a.patient?.personal?.name || a.patientName || "Walk-in Customer";
          bVal = b.patient?.personal?.name || b.patientName || "Walk-in Customer";
        }
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      const endpoint =
        category === "TRANSPLANT" ? "/api/transactions/transplant/delete"
        : category === "SERVICE" ? "/api/transactions/service/delete"
        : category === "MEDICINE" ? "/api/transactions/medicine/delete"
  ...
  const openBillGenerator = (data) => {
    try {
      ...
      if (isRevenueTransaction && (!hasCategory || data.transactionCategory === "TRANSPLANT") && data.patient) {
  ...
  const applyQuickFilter = (preset) => {
    ...
    switch (preset) {
      ...
      case "yesterday":
        ...
        break;
      case "week":
        const weekAgo = new Date(date.setDate(date.getDate() - 7)).toISOString().split("T")[0];
        setFilters({ ...filters, dateFrom: weekAgo, dateTo: getTodayDate() });
        break;
      case "month":
        const monthAgo = new Date(date.setMonth(date.getMonth() - 1)).toISOString().split("T")[0];
              ...
              <div className="flex gap-2 sm:gap-3 w-full lg:w-auto">
                ...
                <button
                  ...
                  className={`p-2 sm:p-2.5 rounded-xl transition-all shrink-0 flex items-center gap-2 ${
                    showFilters ? "bg-indigo-50 text-indigo-600 ring-2 ring-indigo-200" : "bg-gray-50 hover:bg-gray-100 text-gray-600"
                  }`}
                >
              ...
              <div className="px-4 sm:px-6 pb-4 sm:pb-5 border-t border-gray-100 pt-4 sm:pt-5 bg-linear-to-b from-indigo-50/30 to-white">
                ...
                <div className="grid grid-cols-1 xs:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-3 sm:gap-4">
                  <Select
                    ...
                    options={[{ value: "", label: "All Branches" }, ...tenantBranches.map((b) => ({ value: b, label: b }))]}
                    icon={Building2}
                  />
                  ...
                  <Select
                    ...
                    options={[{ value: "", label: "All Methods" }, ...PAYMENT_METHODS.map((m) => ({ value: m, label: m.toUpperCase() }))]}
                    icon={CreditCard}
                  />
                  {(activeCategory === "TRANSPLANT" || activeCategory === "SERVICE") && (
                    <Select
                      ...
                      onChange={(val) => setFilters({ ...filters, procedure: val })}
                      options={[
                        { value: "", label: "All Procedures" },
                        ...(activeCategory === "TRANSPLANT" ? TRANSPLANT_PROCEDURES : SERVICE_PROCEDURES).map((p) => ({ value: p, label: p })),
                      ]}
                    />
                  )}
                </div>
