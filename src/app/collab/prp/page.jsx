...
// ─── main component ──────────────────────────────────────────────────────────
export default function PRPGFCPage() {
  ...
  const [activeType, setActiveType]         = useState("all");   // all | PRP | GFC
  ...
  const fetchData = useCallback(
    async (silent = false) => {
      ...
      try {
        const res  = await fetch(`/api/collab/prp?date=${selectedDate}`);
  ...
  const handleAdd = async () => {
    ...
    setSaving(true);
    try {
      const res = await fetch("/api/collab/prp", {
        method: "POST",
  ...
  const handleExport = async () => {
    setExporting(true);
    try {
      const res  = await fetch(`/api/collab/prp?from=${exportFrom}&to=${exportTo}`);
      const data = await res.json();
      if (!res.ok) throw new Error(data.error || "Fetch failed");
      ...
      <main className="flex-1 p-4 lg:p-8 space-y-6 min-w-0">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-gray-500 mt-0.5">Track daily PRP and GFC treatments</p>
          </div>
