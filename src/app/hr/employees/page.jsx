...
export default function HREmployeesPage() {
  ...
  const filtered = useMemo(() => {
    ...
    if (filters.minPatients) list = list.filter((item) => item.totalPatient >= Number(filters.minPatients));
    if (filters.maxPatients) list = list.filter((item) => item.totalPatient <= Number(filters.maxPatients));
    if (filters.minAmount)   list = list.filter((item) => item.amountReceived >= Number(filters.minAmount));
    if (filters.maxAmount)   list = list.filter((item) => item.amountReceived <= Number(filters.maxAmount));
    if (filters.minGrafts)   list = list.filter((item) => (item.graftsImplanted || 0) >= Number(filters.minGrafts));
    if (filters.maxGrafts)   list = list.filter((item) => (item.graftsImplanted || 0) <= Number(filters.maxGrafts));
    list.sort((a, b) => {
  ...
  const clearFilters = () =>
    setFilters({ search: "", category: "", status: "", minPatients: "", maxPatients: "", minAmount: "", maxAmount: "", minGrafts: "", maxGrafts: "" });
  ...
  const activeFilterChips = useMemo(() => {
    ...
    if (filters.minAmount)   chips.push({ k: "minAmount",   label: `Min Amount: ${formatCurrency(filters.minAmount)}` });
    if (filters.maxAmount)   chips.push({ k: "maxAmount",   label: `Max Amount: ${formatCurrency(filters.maxAmount)}` });
  ...
  const hasGrafts          = ["Doctor", "Technician", "Implanter", "Others"].includes(filters.category || selectedCategory);
  const hasReadyForSurgery = ["Agent", "Counsellor"].includes(filters.category || selectedCategory);
      ...
      <main className="flex-1 flex flex-col">
        ...
        <header className="bg-white shadow-sm sticky top-0 z-10">
          <div className="px-6 py-4 flex items-center justify-between">
            ...
            <Link href="/hr/employees/add-employee"
              className="inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-violet-600 text-white hover:bg-violet-700 transition-colors text-sm font-medium">
            ...
            <div className="px-6 pb-3 flex flex-wrap gap-2">
              {activeFilterChips.map((c) => (
                <span key={c.k} className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-indigo-50 text-indigo-700 text-sm font-medium border border-indigo-200">
                  {c.label}
                  <button onClick={() => removeChip(c.k)} className="hover:text-indigo-900 transition-colors">
                    <X className="w-3 h-3" />
                  </button>
                </span>
              ))}
              <button onClick={clearFilters} className="text-sm text-gray-600 hover:text-gray-900 underline transition-colors">
                Clear all
              </button>
            </div>
          )}
        ...
        <div className="px-6 pt-6">
          <div className="flex gap-2 overflow-x-auto pb-2">
            {CATEGORY_OPTIONS.map((cat) => (
              <button
                key={cat}
                onClick={() => { setSelectedCategory(cat); setFilters((f) => ({ ...f, category: cat })); }}
        ...
        <div className="px-6 pt-6">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            {hasGrafts          && <SummaryCard title="Total Grafts"       value={formatNumber(totals.graftsImplanted)}  icon={<TrendingUp className="w-5 h-5" />}  color="purple" />}
            ...
            <SummaryCard title="Total Revenue"    value={formatCurrency(totals.amountReceived)} icon={<IndianRupee className="w-5 h-5" />} color="indigo" />
          </div>
        </div>
            ...
            <div className="overflow-x-auto">
              <table className="min-w-full text-sm">
                ...
                <tbody className="divide-y divide-gray-100">
                  {rows.length === 0 ? (
                    <tr>
                      <td colSpan={hasGrafts || hasReadyForSurgery ? 8 : 7} className="py-12 text-center text-gray-500">
                    ...
                    rows.map((item, idx) => {
                      ...
                      return (
                        <tr key={idx} className="hover:bg-gray-50 transition-colors">
                          ...
                          {hasGrafts          && <td className="px-6 py-4 text-gray-700">{formatNumber(item.graftsImplanted || 0)}</td>}
                          {hasReadyForSurgery && <td className="px-6 py-4 text-gray-700">{item.readyForSurgery || 0}</td>}
                          <td className="px-6 py-4 text-gray-900 font-medium">{formatCurrency(item.amountReceived)}</td>
                          <td className="px-6 py-4 text-gray-600">{formatCurrency(avg)}</td>
                          <td className="px-6 py-4">
                            <div className="flex items-center gap-1.5">
                              <Link href={`/hr/employees/update/${item._id}`}
                              ...
                              <button
                                ...
                                className="p-1.5 rounded-lg hover:bg-red-50 text-red-500 transition-colors disabled:opacity-40" title="Delete">
                ...
                {rows.length > 0 && (
                  <tfoot className="bg-gray-50 border-t-2 border-gray-200 font-semibold">
                    <tr>
                      ...
                      {hasGrafts          && <td className="px-6 py-4 text-gray-900">{formatNumber(totals.graftsImplanted)}</td>}
                      {hasReadyForSurgery && <td className="px-6 py-4 text-gray-900">{totals.readyForSurgery}</td>}
                      <td className="px-6 py-4 text-gray-900">{formatCurrency(totals.amountReceived)}</td>
                      <td className="px-6 py-4 text-gray-600">
                        {totals.totalPatient > 0 ? formatCurrency(totals.amountReceived / totals.totalPatient) : "—"}
                      </td>
                      <td className="px-6 py-4" />
                    </tr>
                  </tfoot>
          ...
          <div className="fixed inset-0 z-50">
            ...
            <div className="absolute right-0 top-0 h-full w-full max-w-md bg-white shadow-xl flex flex-col">
                ...
                <div className="p-6 space-y-6">
                  <Section title="Category" icon={<Filter className="w-4 h-4" />}>
                    <Field label="Staff Category">
                      <Select
                        ...
                        onChange={(v) => { setFilters((f) => ({ ...f, category: v })); setSelectedCategory(v || "Doctor"); }}
                        options={[{ label: "All Categories", value: "" }, ...CATEGORY_OPTIONS.map((c) => ({ label: c, value: c }))]}
                      />
                    </Field>
                  </Section>

                  <Section title="Status" icon={<Activity className="w-4 h-4" />}>
                  ...
                  <Section title="Patient Count" icon={<Users className="w-4 h-4" />}>
                    <div className="grid grid-cols-2 gap-4">
                      <Field label="Min Patients"><FilterInput type="number" value={filters.minPatients} onChange={(v) => setFilters((f) => ({ ...f, minPatients: v }))} placeholder="0" /></Field>
                      <Field label="Max Patients"><FilterInput type="number" value={filters.maxPatients} onChange={(v) => setFilters((f) => ({ ...f, maxPatients: v }))} placeholder="100" /></Field>
                    </div>
                  </Section>

                  <Section title="Amount Received" icon={<IndianRupee className="w-4 h-4" />}>
                    <div className="grid grid-cols-2 gap-4">
                      <Field label="Min Amount (₹)"><FilterInput type="number" value={filters.minAmount} onChange={(v) => setFilters((f) => ({ ...f, minAmount: v }))} placeholder="0" /></Field>
                      <Field label="Max Amount (₹)"><FilterInput type="number" value={filters.maxAmount} onChange={(v) => setFilters((f) => ({ ...f, maxAmount: v }))} placeholder="500000" /></Field>
                    </div>
                  </Section>
                  ...
                  {hasGrafts && (
                    <Section title="Grafts Implanted" icon={<TrendingUp className="w-4 h-4" />}>
                      <div className="grid grid-cols-2 gap-4">
                        <Field label="Min Grafts"><FilterInput type="number" value={filters.minGrafts} onChange={(v) => setFilters((f) => ({ ...f, minGrafts: v }))} placeholder="0" /></Field>
                        <Field label="Max Grafts"><FilterInput type="number" value={filters.maxGrafts} onChange={(v) => setFilters((f) => ({ ...f, maxGrafts: v }))} placeholder="30000" /></Field>
                      </div>
                    </Section>
                  )}
                </div>
              ...
              <div className="px-6 py-4 border-t bg-gray-50 flex items-center justify-between gap-3">
                <button onClick={clearFilters} className="flex-1 px-4 py-2.5 rounded-lg border border-gray-300 bg-white text-gray-700 hover:bg-gray-50 transition-colors font-medium">
                  Reset All
                </button>
                <button onClick={() => setDrawerOpen(false)} className="flex-1 px-4 py-2.5 rounded-lg bg-indigo-600 text-white hover:bg-indigo-700 transition-colors font-medium">
                  Apply Filters
                </button>
              </div>
            </div>
...
function Th({ label, sortKey, sort, onSort }) {
  ...
  return (
    <th
      className={`px-6 py-3 text-left font-medium ${isSortable ? "cursor-pointer select-none hover:bg-gray-100" : ""}`}
...
function FilterInput({ type = "text", value, onChange, placeholder }) {
  return (
    <input
      type={type} value={value} placeholder={placeholder}
      onChange={(e) => onChange(e.target.value)}
      className="w-full rounded-lg border border-gray-300 px-3 py-2.5 text-sm shadow-sm focus:outline-none focus:ring-2 focus:ring-indigo-200 focus:border-indigo-500 transition-colors"
    />
  );
}
...
function StatusBadge({ status }) {
  ...
  }[status] || { label: "Unknown", className: "bg-gray-100 text-gray-800 border-gray-200" };
  return (
