        ...
        <div className="sticky top-0 bg-white border-b border-gray-100 px-6 py-4 flex items-center justify-between rounded-t-2xl z-10">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-full bg-indigo-100 flex items-center justify-center">
              <span className="text-indigo-700 font-bold text-sm">{c.name?.slice(0,2).toUpperCase()}</span>
            </div>
            <div>
          ...
          {(c.communication || c.technicalKnowledge || c.personality || c.motivation || c.stability) && (
            <section>
              <h3 className="text-xs font-bold text-gray-400 uppercase tracking-widest mb-3">
                Evaluation {avg && <span className="text-indigo-600 normal-case font-semibold">· Avg {avg}/10</span>}
              </h3>
              ...
              {c.hrComments && (
                <div className="mt-3 p-3 bg-indigo-50 rounded-lg">
                  <p className="text-xs text-indigo-600 font-medium mb-1">HR Comments</p>
...
function KpiTile({ label, value, accent }) {
  ...
  return (
    <div className={`rounded-xl border p-4 ${accents[accent] || accents.indigo}`}>
...
export default function AppliedPage() {
  ...
  const fetchCandidates = useCallback(async () => {
    setLoading(true);
    try {
      const res  = await fetch("/api/hr/candidates?source=direct");
  ...
  const STATUS_TABS = [
    { val: "",                     label: "All",       icon: Globe,       accent: "indigo" },
      ...
      <main className="flex-1 overflow-auto">
        ...
        <div className="bg-white border-b border-gray-200 px-6 py-5 sticky top-0 z-10 shadow-sm">
          <div className="flex items-center justify-between gap-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-lg font-bold text-gray-900 leading-tight">Applied Online</h1>
                <p className="text-xs text-gray-500 mt-0.5">Candidates who applied via direct link</p>
              </div>
            </div>
            <div className="text-right">
              <p className="text-2xl font-bold text-indigo-600">{candidates.length}</p>
        ...
        <div className="p-6 space-y-5">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3">
            <KpiTile label="Total Applied"  value={kpi.total}     accent="indigo" />
            <KpiTile label="Selected"       value={kpi.selected}  accent="emerald" />
            <KpiTile label="Rejected"       value={kpi.rejected}  accent="red" />
            <KpiTile label="Scheduled"      value={kpi.scheduled} accent="blue" />
            <KpiTile label="Freshers"       value={kpi.freshers}  accent="violet" />
          </div>
          ...
          <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
            ...
            {loading ? (
              ...
              <div className="text-center py-16 text-gray-400">
                ...
                <p className="text-sm">No applied candidates found</p>
              </div>
            ) : (
                    ...
                    {paged.map((c, i) => {
                      ...
                      return (
                        <tr key={c._id} className="hover:bg-indigo-50/30 transition-colors group">
                          ...
                          <td className="px-4 py-3">
                            <div className="flex items-center gap-2.5">
                              <div className="w-8 h-8 rounded-full bg-indigo-100 flex items-center justify-center shrink-0">
                                <span className="text-indigo-700 font-bold text-xs">{c.name?.slice(0,2).toUpperCase()}</span>
                              </div>
                              <div>
                                <p className="font-semibold text-gray-800 text-sm leading-none">{c.name}</p>
                                {c.email && <p className="text-xs text-gray-400 mt-0.5 truncate max-w-35">{c.email}</p>}
                              </div>
                            </div>
                          </td>
                          ...
                          <td className="px-4 py-3 whitespace-nowrap">
                            {c.assignedHr?.name
                              ? <span className="text-xs bg-violet-50 text-violet-700 px-2 py-0.5 rounded-full font-medium">{c.assignedHr.name}</span>
