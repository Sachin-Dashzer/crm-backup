...
import { X, Loader2, Ban, Trash2, Wallet } from "lucide-react";
...
const inputClass =
  "w-full rounded-xl border border-slate-300 px-3 py-2.5 text-sm outline-none transition placeholder:text-slate-300 focus:border-emerald-500 focus:ring-2 focus:ring-emerald-100 disabled:bg-slate-50 disabled:text-slate-400";
const labelClass = "mb-1.5 block text-sm font-semibold text-slate-700";

export default function EditAdvanceModal({ advance, onClose, onSuccess, toast }) {
  const isRecovery = advance.direction === "IN";
  ...
  const remove = async () => {
    const ok = window.confirm(
      `Permanently delete this ${isRecovery ? "recovery" : "advance"} row (${formatCurrency(advance.amount)})?\n\nThis cannot be undone.`,
    );
    if (!ok) return;
    setSubmitting(true);
    try {
          ...
          <div className="flex min-w-0 items-center gap-3">
            ...
            <div className="min-w-0">
              <h3 className="truncate text-base font-bold text-slate-900 sm:text-lg">
                Edit {isRecovery ? "Recovery" : "Advance"}
              </h3>
          ...
          <div className="space-y-5 p-4 sm:p-6">
            ...
            <div className="rounded-xl border border-slate-200 bg-white p-4">
              <div className="flex items-center justify-between gap-3">
                <p className="truncate text-sm font-bold text-slate-900">{advance.party?.label}</p>
                {advance.isCancelled ? (
              ...
              <p className="mt-1 text-xs text-slate-500">{isRecovery ? "Recovery" : "Paid out"}</p>
            </div>
