// scripts/rent-payables-apr-jul-2026.mjs
//
// Bulk-imports Rent payables for April, May, June and July 2026 — one self-contained file, same
// pattern as scripts/april-rent-payables.mjs (embedded JSON + import logic together).
//
// UPDATED from the previous "May-Aug 2026" version to Dashzer's corrected Rent Data.xlsx:
//   - Now covers April-July 2026 (was May-Aug) — April is newly included, August is dropped.
//   - Adds landlord names ("Paid To (Landlord)"), folded into each entry's remarks.
//   - Backend Basement/upper ground floor TDS changes from 31.2% (Apr-May, landlord Satpal
//     Singh) to no TDS (Jun-Jul, landlord Manjeet Laudha/Pramod) — see meta.flags below.
//
// The embedded PAYLOAD was generated by parsing Rent Data.xlsx directly (all rows, single
// "Sheet1" keyed by Period Month/Period Year) — not hand-transcribed — then verified with
// --dump-json and a dry run before being embedded here. See PAYLOAD.meta for the source,
// totals and the flagged rows.
//
// Usage:
//   node scripts/rent-payables-apr-jul-2026.mjs                                   # dry run
//   node scripts/rent-payables-apr-jul-2026.mjs --dump-json                       # write PAYLOAD out, no DB
//   node scripts/rent-payables-apr-jul-2026.mjs --apply                          # write (blocked while flags unconfirmed)
//   node scripts/rent-payables-apr-jul-2026.mjs --apply --confirm-flagged        # write, flags acknowledged
//   node scripts/rent-payables-apr-jul-2026.mjs --apply --confirm-flagged --months=5,6,7
//   node scripts/rent-payables-apr-jul-2026.mjs --apply --allow-duplicates       # only if re-running on purpose
//
// NOTE on April: if scripts/april-rent-payables.mjs already ran successfully against this
// database, the April rows here will hit the monthly-duplicate guard below and be skipped
// (or blocked without --allow-duplicates) — that is the correct, expected outcome, not a bug.
// The guard exists specifically so the same month can't be imported twice.

import mongoose from "mongoose";
import fs from "fs";
import { EXPENSE_CATEGORY_TREE, TDS_TAX_TYPES } from "../src/constants/expenseCategories.js";
...
// ═══════════════════════════════════════════════════════════════════════════════
// THE DATA — parsed from Rent Data.xlsx (April, May, June, July 2026). Rates in each entry are
// whole-number percent (already x100 from the sheet's percent-formatted cells) but are METADATA
// ONLY — the money actually written is each entry's own gstAmount/tdsAmount/vendorPayable, taken
// directly from the sheet's amount columns.
// ═══════════════════════════════════════════════════════════════════════════════
const PAYLOAD = {
  "meta": {
    "source": "Rent Data.xlsx (single sheet \"Sheet1\", column-driven by Period Month/Period Year \u2014 not one sheet per month)",
    "sheets": [
      "April 2026",
      "May 2026",
      "June 2026",
    ...
    "flags": {
      "rate_unit_converted": "Excel GST/TDS Rate % cells are percent-formatted (0.18 stored = 18%). Rates were multiplied by 100 to match what computeTaxBreakdown expects (whole-number percent). The money actually written uses each row's own GST Amount / TDS Amount columns directly (direct amount wins over rate), so the rate fields are metadata / display only.",
      "needs_confirmation": [
        {
          "subType": "Rent-Backend 1st Floor",
          "issue": "TDS rate 31.2% every month (April-July). Statutory TDS on rent (s.194-I) is 10%. Repeats the same flag raised for the prior imports."
        },
        {
          "subType": "Rent-Backend Basement / Rent-Backend upper ground floor",
          "issue": "TDS 31.2% in April-May (landlord: Satpal Singh), then Include TDS = No from June onward (landlord changes to Manjeet Laudha / Pramod). Confirm the landlord change and the resulting drop to zero TDS are both correct and not a data-entry gap."
        }
      ],
      "excluded": []
    }
  ...
  "entries": [
    ...
    },
    {
      ...
      "dueDateRaw": "01.04.2026",
      "branch": "Delhi",
      "remarks": "April rent \u2014 Landlord: Bajinder kaur Bhasinr/Stpal",
      "landlord": "Bajinder kaur Bhasinr/Stpal",
      "includeGST": false,
      "gstRate": 0,
      "gstAmount": 0,
    ...
    },
    {
      ...
      "dueDateRaw": "01.04.2026",
      "branch": "Delhi",
      "remarks": "April rent \u2014 Landlord: MANU ENTERPRISES",
      "landlord": "MANU ENTERPRISES",
      "includeGST": true,
      "gstRate": 18.0,
      "gstAmount": 15120.0,
    ...
    },
    {
      ...
      "dueDateRaw": "01.04.2026",
      "branch": "Delhi",
      "remarks": "April rent \u2014 Landlord: MANSI ENTERPRISES",
      "landlord": "MANSI ENTERPRISES",
      "includeGST": true,
      "gstRate": 18.0,
      "gstAmount": 15120.0,
      "invoiceTotal": 99120.0,
    ...
    },
    {
      ...
      "dueDateRaw": "01.05.2026",
      "branch": "Delhi",
      "remarks": "May rent \u2014 Landlord: MANU ENTERPRISES",
      "landlord": "MANU ENTERPRISES",
      "includeGST": true,
      "gstRate": 18.0,
      "gstAmount": 18144.0,
    ...
    },
    {
      ...
      "dueDateRaw": "01.05.2026",
      "branch": "Delhi",
      "remarks": "May rent \u2014 Landlord: MANSI ENTERPRISES",
      "landlord": "MANSI ENTERPRISES",
      "includeGST": true,
      "gstRate": 18.0,
      "gstAmount": 18144.0,
    ...
    },
    {
      ...
      "dueDateRaw": "01.06.2026",
      "branch": "Delhi",
      "remarks": "June rent \u2014 Landlord: MANU ENTERPRISES",
      "landlord": "MANU ENTERPRISES",
      "includeGST": true,
      "gstRate": 18.0,
      "gstAmount": 15876.0,
    ...
    },
    {
      ...
      "dueDateRaw": "01.06.2026",
      "branch": "Delhi",
      "remarks": "June rent \u2014 Landlord: MANSI ENTERPRISES",
      "landlord": "MANSI ENTERPRISES",
      "includeGST": true,
      "gstRate": 18.0,
      "gstAmount": 15876.0,
    ...
    },
    {
      ...
      "dueDateRaw": "01.07.2026",
      "branch": "Delhi",
      "remarks": "July rent \u2014 Landlord: MANU ENTERPRISES",
      "landlord": "MANU ENTERPRISES",
      "includeGST": true,
      "gstRate": 18.0,
      "gstAmount": 15876.0,
    ...
    },
    {
      ...
      "dueDateRaw": "01.07.2026",
      "branch": "Delhi",
      "remarks": "July rent \u2014 Landlord: MANSI ENTERPRISES",
      "landlord": "MANSI ENTERPRISES",
      "includeGST": true,
      "gstRate": 18.0,
      "gstAmount": 15876.0,
...
const MONTHS_FILTER = arg("months")
  ? arg("months").split(",").map((s) => parseInt(s.trim(), 10))
  : null;
...
if (DUMP_JSON) {
  ...
  console.log(`Wrote ${out} — ${ENTRIES.length} of ${ALL_ENTRIES.length} row(s) selected.`);
  process.exit(0);
}
...
function validate() {
  ...
  for (const e of ENTRIES) {
    ...
    if (!EXPENSE_CATEGORY_TREE["Rent"]?.includes(e.expenseSubType))
      errors.push(`${where}: "${e.expenseSubType}" is not a valid sub-type under "Rent"`);
    ...
    if (e.tdsAmount > 0) {
      if (!e.tdsCategory) errors.push(`${where}: TDS amount is set but TDS Category is empty`);
      else if (!TDS_TAX_TYPES.includes(e.tdsCategory) || !EXPENSE_CATEGORY_TREE["Taxes"]?.includes(e.tdsCategory))
        errors.push(`${where}: TDS Category "${e.tdsCategory}" is not a valid Taxes sub-type`);
...
function flagRows() {
  ...
  for (const e of ENTRIES) {
    if (e.tdsAmount > 0 && e.tdsRate > FLAG_TDS_RATE_ABOVE) {
      flags.push({
        sheet: e.sheet, rowNum: e.rowNum, subType: e.expenseSubType,
        issue: `TDS rate ${e.tdsRate}% on rent — statutory rent TDS (s.194-I) is 10%. Confirm this is intentional.`,
        atStatedAmount: e.tdsAmount, at10pct: r2(e.baseAmount * 0.1),
      });
    }
...
function buildRentPayable(e) {
  ...
  return {
    ...
    ...(hasTds
      ? { tdsLink: { role: "PARENT", linkedId: null, tdsRate: e.tdsRate, tdsAmount: e.tdsAmount, grossAmount: e.invoiceTotal } }
      : {}),
    log: [{
      action: "Created",
      newValue: String(e.vendorPayable),
      note: note ? `${note} [bulk import: ${e.sheet}]` : `Bulk import from Excel [${e.sheet}]`,
...
function buildTdsPayable(e, parentId) {
  return {
    ...
    tdsLink: { role: "TDS", linkedId: parentId, tdsRate: e.tdsRate, tdsAmount: e.tdsAmount, grossAmount: e.invoiceTotal },
    log: [{
      action: "Created",
...
async function run() {
  ...
  console.log(`Source: ${PAYLOAD.meta.source} (${PAYLOAD.meta.sheets.join(", ")})`);
  console.log(`Rows: ${ENTRIES.length} rent payables + ${withTds.length} TDS payables = ${ENTRIES.length + withTds.length} documents`);
  if (MONTHS_FILTER) console.log(`Month filter applied: ${MONTHS_FILTER.join(",")}`);
  console.log("=".repeat(90) + "\n");
  ...
  const conserved = Math.abs(vendorTotal + tdsTotal - invoiceTotal) < 0.01 * ENTRIES.length;
  console.log("--- TOTALS ---");
  ...
  console.log(`  Conservation check         : ${inr(vendorTotal + tdsTotal)} ${conserved ? "== invoice total  OK" : "!= invoice total  MISMATCH"}`);
  if (!conserved) {
  ...
  ENTRIES.forEach((e) => {
    if (e.sheet !== currentSheet) { currentSheet = e.sheet; console.log(`\n  ${currentSheet}`); }
    const flag = flaggedSet.has(`${e.sheet}#${e.rowNum}`) ? "  <-- FLAGGED" : "";
    const tds = e.tdsAmount > 0 ? `  +TDS ${inr(e.tdsAmount)} @${e.tdsRate}%` : "  (no TDS)";
    console.log(`    ${String(e.rowNum).padStart(3)}  ${e.expenseSubType.padEnd(34)} ${inr(e.vendorPayable).padStart(14)}${tds}${flag}`);
  });

  if (flags.length) {
    console.log("\n" + "!".repeat(90));
    console.log(`FLAGGED — ${flags.length} row(s) worth a second look before writing`);
    console.log("!".repeat(90));
    flags.forEach((f) => {
      console.log(`  ${f.sheet} row ${f.rowNum} (${f.subType}): ${f.issue}`);
      if (f.at10pct !== undefined) console.log(`      at stated rate: ${inr(f.atStatedAmount)}   at 10%: ${inr(f.at10pct)}`);
    });
    console.log("");
    if (APPLY && !CONFIRM_FLAGGED) {
      console.error("Refusing to apply. If these figures are correct, re-run with --confirm-flagged.");
      console.error("If they are not, fix PAYLOAD (or regenerate it from the sheet) and re-run.");
      process.exit(1);
    }
  ...
  for (const e of ENTRIES) {
    const existing = await Payable.findOne({
      "payee.kind": e.payeeKind, "payee.label": e.expenseSubType, purpose: e.purpose,
      "period.month": e.period.month, "period.year": e.period.year,
    }).select("_id totalAmount isCancelled").lean();
    if (existing) dupes.push({ sheet: e.sheet, rowNum: e.rowNum, subType: e.expenseSubType, id: String(existing._id), amount: existing.totalAmount, cancelled: !!existing.isCancelled });
  }

  if (dupes.length) {
    console.log(`\nWARNING — ${dupes.length} row(s) already have a payable for that month:`);
    dupes.forEach((d) => console.log(`  ${d.sheet} row ${d.rowNum}  ${d.subType.padEnd(34)} -> ${d.id} (${inr(d.amount)})${d.cancelled ? " [cancelled]" : ""}`));
  ...
  if (!APPLY) {
    console.log("DRY RUN — nothing written. Reconcile the totals above, then re-run with --apply.");
    await mongoose.disconnect();
    return;
  }
  ...
  for (const e of ENTRIES) {
    if (dupeKeys.has(`${e.sheet}#${e.rowNum}`) && !ALLOW_DUPES) {
      skippedAsDupe.push({ sheet: e.sheet, rowNum: e.rowNum, subType: e.expenseSubType });
      continue;
    }
    try {
      if (!(e.tdsAmount > 0)) {
        const doc = await Payable.create(buildRentPayable(e));
        created.push({ sheet: e.sheet, rowNum: e.rowNum, subType: e.expenseSubType, ids: [String(doc._id)] });
      } else {
        ...
        try {
          await dbSession.withTransaction(async () => {
            ...
            tdsId = tds._id;
            await Payable.updateOne({ _id: parentId }, { $set: { "tdsLink.linkedId": tdsId } }, { session: dbSession });
          });
        } finally {
          await dbSession.endSession();
        }
        created.push({ sheet: e.sheet, rowNum: e.rowNum, subType: e.expenseSubType, ids: [String(parentId), String(tdsId)] });
      }
      console.log(`  ${e.sheet} row ${String(e.rowNum).padStart(3)}  ${e.expenseSubType}  OK`);
    } catch (err) {
      const reason = err?.code === 11000 ? "duplicate — a payable already exists for this rent unit this month" : err?.message || String(err);
      failed.push({ sheet: e.sheet, rowNum: e.rowNum, subType: e.expenseSubType, reason });
      console.log(`  ${e.sheet} row ${String(e.rowNum).padStart(3)}  ${e.expenseSubType}  FAILED: ${reason}`);
    }
  }

  const docCount = created.reduce((s, c) => s + c.ids.length, 0);
  console.log(`\nCreated ${docCount} payable document(s) from ${created.length} row(s).`);
  if (skippedAsDupe.length) console.log(`Skipped ${skippedAsDupe.length} row(s) already imported.`);
  if (failed.length) {
    console.log(`\n${failed.length} row(s) failed:`);
    failed.forEach((f) => console.log(`  ${f.sheet} row ${f.rowNum}  ${f.subType}: ${f.reason}`));
  ...
  fs.writeFileSync(reportPath, JSON.stringify({ source: PAYLOAD.meta.source, invoiceTotal, vendorTotal, tdsTotal, created, skippedAsDupe, failed }, null, 2));
